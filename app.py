...
def seed():
  ...
  for data in venues:
    ...
    venueModel = Venue(name=data['name'],genres=genres
      ,city=data['city'],state=data['state'],address=data['address'],phone=data['phone']
      ,website=data['website'], facebook_link=data['facebook_link']
      , seeking_talent=data['seeking_talent'], seeking_description=data.get('seeking_description')
  ...
  for data in artists:
    ...
    artistModel = Artist(name=data['name'],genres=genres
      ,city=data.get('city'),state=data.get('state'),phone=data.get('phone')
      ,website=data.get('website'), facebook_link=data.get('facebook_link')
      ,seeking_venue=data.get('seeking_venue'),seeking_description=data.get('seeking_description')
  ...
  for show in shows:
    show = Show(venue_id=show["venue_id"], artist_id=show["artist_id"], start_time=show["start_time"])
    db.session.add(show)
  db.session.commit()
...
def venues():
  ...
  for city in cities:
    ...
    data.append({
      ...
      "state":city[1],
      "venues": venues,
      "num_upcoming_shows": Show.query.filter(Show.start_time > current_time).count()
    })
...
def search_venues():
  ...
  for venue in venues:
    data.append({
      "id": venue.id,
      "name": venue.name,
      "num_upcoming_shows": Show.query.filter(Show.start_time > current_time)
